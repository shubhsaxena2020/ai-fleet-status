...
| AFS-04 | LOW | CONFIRMED/FIXED + DOCUMENTED | lib/sessions.js | On Unix/macOS `ps` supplies no creation timestamp, so `sessionId()` fell back to `scope:toolId:rootPid:?` — a PID-only identity. If PID 123 is reused before a creation timestamp exists, the old and new invocation share the same session id and lifecycle cache entry, hiding a restart. | **Two-layer mitigation.** (1) Content-fingerprint fallback: when `creationTime` is null but a live process exists, the id embeds `fp:<ppid>:<cmdlineHash>` (FNV-1a of the root command line) instead of `?`, so PID reuse with a different parent/argv stays distinct — and a still-running session keeps the SAME id across polls (continuity preserved). (2) Monotonic `pollSeq` tiebreaker (added in 0.3.3): when there is NO process data at all (rootProc null, so neither timestamp nor fingerprint is available), the `?` branch now embeds `#<pollSeq>` so distinct polls yield DISTINCT ids and a reused PID no longer collapses two invocations into one cache entry — the audit's exact "PID reused before a creation timestamp exists" scenario. Trade-off (documented): with no process data, a still-running session is treated as new each poll (continuity sacrificed), which is strictly preferable to merging two different invocations. **Residual (documented, cannot be fully closed without an OS creation timestamp on Unix):** a PID reused with the *same* parent AND identical command line AND no process data still collapses. Regression: `test/audit-afs03-05.test.js` (AFS-04 group: 3 tests — distinct fingerprints; two PID-123 invocations distinct across polls; pollSeq tiebreaker `?#1`/`?#2`; same fingerprint keeps continuity; legacy no-pollSeq call stays `?`). |
