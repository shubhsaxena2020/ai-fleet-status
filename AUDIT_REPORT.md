...
| AFS-04 | LOW | CONFIRMED/PARTIAL FIX + DOCUMENTED | lib/sessions.js | On Unix/macOS `ps` supplies no creation timestamp, so `sessionId()` fell back to `scope:toolId:rootPid:?` — a PID-only identity. If PID 123 is reused before a creation timestamp exists, the old and new invocation share the same session id and lifecycle cache entry, hiding a restart. | Added a content-fingerprint fallback: when `creationTime` is null, the id embeds `fp:<ppid>:<cmdlineHash>` (FNV-1a of the root command line) instead of `?`, so PID reuse with a different parent/argv stays distinct. **Residual limitation (documented):** a PID reused with the *same* parent AND identical command line still collapses — there is no creation timestamp on Unix to split them. Regression: `test/audit-afs03-05.test.js` (`sessionId` distinct for different fingerprints; `buildFleet` keeps two PID-123 invocations distinct across polls; identical parent+argv reusing PID collides as expected). |
