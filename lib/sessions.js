...
//
// To mitigate PID reuse without a real timestamp, callers pass a `fingerprint`
// (parent-PID + command-line content hash) for the live process. When present we
// embed it instead of the bare `?`, so two invocations with the same PID but
// different parent or argv stay distinct. This is correct for the NORMAL Unix case
// and preserves continuity of a still-running session across polls.
//
// Residual (AFS-04): when there is NO process data at all for the root (rootProc is
// null, so neither creationTime nor a fingerprint is available), the id would
// otherwise collapse to a bare `?` — so two DIFFERENT invocations that reuse the
// same PID at different polls would share one id and one lifecycle cache entry,
...
function sessionId(scope, toolId, rootPid, creationTime, fingerprint, pollSeq) {
  let t;
...
function buildFleet(processes, detectors, options) {
  ...
  for (const rootPid of roots) {
    ...
    // Without a creation timestamp (Unix/macOS), fall back to a content fingerprint
    // (parent PID + command-line hash) so PID reuse with different argv/parent is
    // still distinguishable (AFS-04). Best-effort; identical argv+ppid reusing a PID
    // cannot be split (documented residual limitation).
    ...
    const sid = sessionId(rootProc && rootProc.scope, detector.id, rootPid, creationTime, fingerprint, pollSeq);
