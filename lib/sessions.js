...
// different parent or argv stay distinct. This is a BEST-EFFORT heuristic: if a
// PID is reused AND the new process has the same parent AND an identical command
// line, the two invocations cannot be told apart and are merged — that residual
// case is acknowledged and documented in AUDIT_REPORT.md (AFS-04).
function sessionId(scope, toolId, rootPid, creationTime, fingerprint) {
  let t;
...
function buildFleet(processes, detectors, options) {
  ...
  for (const rootPid of roots) {
    ...
    const sid = sessionId(rootProc && rootProc.scope, detector.id, rootPid, creationTime, fingerprint);
