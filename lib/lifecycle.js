...
class SessionLifecycle {
  ...
  reconcile(fleet) {
    ...
    // Defensive: a null/undefined/malformed fleet must NOT throw
    // (`Cannot read properties of null (reading 'sessions')` / `fleet.sessions is
    // not iterable`). A bad fleet (or a prior step that failed to build one) would
    // otherwise abort the entire poll — the AFS-01 impact class: one bad fleet
    // kills detection/notification for ALL sessions. Degrade to "no current
    // sessions" (everything previously live is reported ended) instead of throwing.
    const sessions = (fleet && Array.isArray(fleet.sessions)) ? fleet.sessions : [];
