...
// Dispatch by platform: Linux uses /proc/<pid>/stat starttime; macOS uses
// proc_pidinfo/ps lstart; everything else returns null (keeps the prior
// null behavior). Both Unix paths feed the SAME `startMs` session-id tier, so a
// real start epoch disambiguates PID reuse on macOS exactly as it does on Linux.
const { getMacOsProcessStartTime } = require('./macos-starttime');

function getProcessStartTimeMs(pid) {
  if (process.platform === 'linux') {
    return estimateUnixStartTimeMs(pid);
  }
...
// Linux-only real start time from /proc/<pid>/stat starttime.
