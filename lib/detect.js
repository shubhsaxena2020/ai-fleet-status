...
function detectOne(process, detector) {
  ...
  const isNative = detector.processNames.has(name);
  ...
  if (!isNative && hosts.length > 0 && typeof commandLine === 'string') {
    ...
    for (const host of hosts) {
      if (!host.interpreters.has(firstBasename)) {
        continue;
      }
      if (host.fragments.some((frag) => programTokens.some((tok) => fragmentInToken(tok, frag)))) {
  ...
  if (!commandLine) {
    return { id: detector.id, toolId: detector.id, kind: 'session', mode: 'unknown', confidence: 'low', reason: 'process name matched; command line unavailable' };
  }
  ...
  if (subcommand && detector.utilSubcommands.has(subcommand)) {
    return null;
  }
  if (subcommand && detector.serviceSubcommands.has(subcommand)) {
    return { id: detector.id, toolId: detector.id, kind: 'service', confidence, reason: 'service subcommand ' + JSON.stringify(subcommand) };
  }

  let mode;
  if (flagBases.some((f) => detector.resumeFlags.has(f))) {
    mode = 'resume';
  } else if (subcommand && detector.delegatedSubcommands.has(subcommand)) {
    mode = 'delegated';
  } else if (flagBases.some((f) => detector.delegatedFlags.has(f))) {
    mode = 'delegated';
  } else if (flagBases.some((f) => detector.interactivePromptFlags.has(f))) {
    mode = 'interactive-prompt';
  } else {
    mode = 'interactive';
  }

  return { id: detector.id, toolId: detector.id, kind: 'session', mode, confidence, reason: `matched by ${isNative ? 'native binary' : 'interpreter script'}` };
}
...
const BUILTIN_SPECS = [
  ...
  },
  {
    ...
    // Native binary (verified); nodeIdentity fragments are harmless backup only.
  ...
  },
  {
    ...
    // npm shim runs node .../@qwen-code/qwen-code/cli-entry.js (verified).
    processNames: ['qwen.exe', 'qwen'],
    interpreterHosted: [{ interpreter: ['node', 'node.exe'], fragments: ['@qwen-code/qwen-code', 'qwen-code', 'cli-entry.js', 'cli.js'] }],
...
function compileDetectorFromConfig(spec) {
  ...
  const out = {
    ...
    serviceSubcommands: new Set((spec.serviceSubcommands || []).map((s) => String(s).toLowerCase())),
    utilSubcommands: new Set((spec.utilSubcommands || []).map((s) => String(s).toLowerCase())),
    delegatedSubcommands: new Set((spec.delegatedSubcommands || []).map((s) => String(s).toLowerCase())),
    delegatedFlags: new Set((spec.delegatedFlags || []).map((s) => String(s).toLowerCase())),
    resumeFlags: new Set((spec.resumeFlags || []).map((s) => String(s).toLowerCase())),
    interactivePromptFlags: new Set((spec.interactivePromptFlags || []).map((s) => String(s).toLowerCase()))
  };
...
function compileTools(configured) {
  if (Array.isArray(configured) && configured.length > 0) {
    ...
    for (const spec of configured) {
      const detector = compileDetectorFromConfig(spec);
      if (detector) {
