...
function detectOne(process, detector) {
  ...
  const hosts = (d && d.interpreterHosted) || [];
...
const BUILTIN_SPECS = [
  ...
  },
  {
    ...
    // Native binary invoked as `rag-service` (verified per project config).
    processNames: ['rag-service', 'rag-service.exe'],
    // Server/daemon modes (excluded from sessions) — verified from rag-service --help.
    ...
    // Interactive prompt flags (if any).
  ...
  },
  {
    id: 'aider',
    displayName: 'Aider',
    // Python agentic coding CLI. Ships as the `aider` binary and also runs as
    // `python -m aider`, so match both the native name and the python-hosted form.
    processNames: ['aider.exe', 'aider'],
    interpreterHosted: [{ interpreter: ['python', 'python3', 'python.exe', 'uv', 'uvx'], fragments: ['aider'] }],
    // `aider` with no flags is the interactive chat session; `-m`/`--message` is the
    // one-shot delegated form.
    utilSubcommands: ['--help', '--version', '--list', 'config', 'setup'],
    delegatedFlags: ['-m', '--message'],
    resumeFlags: []
  },
  {
    id: 'amp',
    displayName: 'Amp',
    // Sourcegraph Amp — native Go binary (verified: invoked as `amp`).
    processNames: ['amp.exe', 'amp'],
    // `amp` with no flags is the interactive agent; `-p`/`--print` is the delegated
    // (non-interactive) form; `-c`/`--continue` resumes the prior session.
    utilSubcommands: ['--help', '--version', 'login', 'logout', 'models', 'update', 'auth'],
    delegatedFlags: ['-p', '--print', '--json'],
    resumeFlags: ['-c', '--continue']
  },
  {
    id: 'crush',
    displayName: 'Crush',
    // Crush — open-source terminal agent (Charm tools, Go binary). Invoked as `crush`.
    processNames: ['crush.exe', 'crush'],
    utilSubcommands: ['--help', '--version', 'config', 'models', 'update'],
    delegatedFlags: ['-p', '--prompt', '--print'],
    resumeFlags: ['--continue', '-c']
  },
  {
    id: 'copilot',
    displayName: 'GitHub Copilot CLI',
    // github/copilot-cli — native binary invoked as `copilot` (and `copilot.exe`).
    // Bare `copilot` launches the interactive chat; this is the official binary name
    // so a native-name match is correct (no bare-fragment over-match risk).
    processNames: ['copilot.exe', 'copilot'],
    utilSubcommands: ['--help', '--version', 'update', 'auth', 'config', 'extension'],
    delegatedFlags: ['-p', '--prompt', '--chat'],
    resumeFlags: ['--continue', '-c']
  },
  {
    id: 'kiro',
